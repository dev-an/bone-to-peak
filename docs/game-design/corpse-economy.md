...
> 적 유형별 Corpse 드롭량. 자세한 적 스탯은 [적 & 웨이브 시스템](./enemy-wave-system.md)을 참고한다.

| 적 유형     | 해당 적                        | 드롭량 | 비고                       |
| ----------- | ------------------------------ | ------ | -------------------------- |
| 일반 근접   | 스웜, 할버디어                 | 1      | 가장 흔한 자원 공급원      |
| 일반 원거리 | 궁수                           | 1      |                            |
| 탱크형      | 방패병                         | 2      | 체력이 높은 대신 보상 증가 |
| 엘리트      | 기사, 마법사, 암살자           | 3      | 웨이브 5 이후 소수 등장    |
| 보스        | 성기사, 드래곤, 대마법사, 용사 | 10     | 대량 확보 기회             |
| 약화 보스   | Ascension 5 재등장 보스        | 5      | 정상 보스의 50%            |
...
- 소환/폭발로 Corpse를 소비하면 즉시 현재 수집 범위(기본 1.5 유닛) 내의 Corpse 자동 수집 재개
    - 수집 재개 시 가장 가까운 Corpse부터 순서대로 수집
    - 수용량이 다시 가득 차면 수집 중단
...
- **합성 보너스**: Refine으로 생성된 T2 미니언은 **전체 스탯 +15%** (HP, 공격력, 이동속도). 상세: [미니언 시스템 → Refine 합성 보너스](./minion-system.md)
    - **슬롯 계산 예시**: 워리어 ×2 (슬롯 2) → 그래브 나이트 (슬롯 1) = 슬롯 1개 반환
    - **T3 슬롯 예시**: 그래브 나이트 (슬롯 1) + 레이스 (슬롯 1) → 데스 나이트 (슬롯 2) = 슬롯 변동 없음
    - **슬롯 부족 시**: 합성 결과물의 슬롯 비용 > 재료 슬롯 합계이면 합성 불가 (현실적으로 발생하지 않음)
    - **결과물 위치**: 첫 번째 재료 미니언의 위치에서 등장

### Refine 효율 분석

직접 Raise와 Refine 경로의 비용 비교. Refine으로 생성된 T2 미니언은 **합성 보너스 +15%**가 적용되어 스탯이 더 높다.

| 결과 미니언        | 직접 Raise 비용 | Refine 총 비용                           | 차이 | 합성 보너스                |
| ------------------ | --------------- | ---------------------------------------- | ---- | -------------------------- |
| 그래브 나이트 (T2) | Corpse 3        | Corpse 4 (워리어 ×2 + 추가 2)            | +1   | HP 115, 공격력 21 (+15%)   |
| 레이스 (T2)        | Corpse 4        | Corpse 7 (아처 ×2 + 추가 3)              | +3   | HP 46, 공격력 29 (+15%)    |
| 본 골렘 (T2)       | Corpse 5        | Corpse 6 (좀비 ×3 + 추가 3)              | +1   | HP 207, 공격력 12 (+15%)   |
| 데스 나이트 (T3)   | 직접 Raise 불가 | Corpse 12 (나이트 3 + 레이스 4 + 추가 5) | —    | T3는 기본 스탯에 이미 포함 |
| 본 드래곤 (T3)     | 직접 Raise 불가 | Corpse 19 (골렘 5 + 아처 ×3 6 + 추가 8)  | —    | T3는 기본 스탯에 이미 포함 |
| 리치 (T3)          | 직접 Raise 불가 | Corpse 18 (레이스 ×2 8 + 추가 10)        | —    | T3는 기본 스탯에 이미 포함 |

> **설계 의도**: 직접 Raise가 Corpse 효율에서 유리하지만, Refine은 **합성 보너스(+15%)**, **소환 슬롯 절약**, **T2 고유 패시브 유지**라는 세 가지 이점이 있다. 슬롯이 부족한 중·후반에 합성의 가치가 극대화된다.
...
| 행동           | 합성 메뉴 열린 상태          | 합성 진행 중 (프로그레스 바)    |
| -------------- | ---------------------------- | ------------------------------- |
| 이동           | 가능                         | 가능 (합성은 네크로맨서에 귀속) |
| 기본 공격      | 불가 (메뉴 조작 중)          | 가능                            |
| Raise (소환)   | 불가                         | 불가                            |
| Explode (폭발) | 불가                         | 가능 (합성 취소 없이)           |
| 포메이션 전환  | 가능                         | 가능                            |
| 피격           | 합성 취소 없음 (메뉴만 닫힘) | **합성 취소** (재료 보존)       |

- 합성 진행 중 네크로맨서가 피격당하면 합성이 취소됨
- 합성 취소 시 재료 미니언과 추가 Corpse 모두 반환
- 재료 미니언은 합성 진행 중 **제자리에 정지** → 적의 공격에 취약

### 합성 레시피 발견

| 방식        | 설명                                                                    |
| ----------- | ----------------------------------------------------------------------- |
| 기본 공개   | T2 합성 레시피는 처음부터 합성 메뉴에 표시                              |
| 조건부 공개 | T3 합성 레시피는 재료 미니언을 **모두 보유한 적이 있을 때** 레시피 해금 |
| 시각적 힌트 | 미해금 레시피는 "???" + 필요 재료 티어 힌트 표시 (예: "T2 + T2 → ???")  |

> T3 레시피 예시: 데스 나이트의 레시피는 그래브 나이트와 레이스를 한 번이라도 동시에 소환한 적이 있어야 합성 메뉴에 등장한다.
...
- E 키를 길게 누르면 게이지가 차오르며, 놓는 시점의 게이지에 해당하는 양만 사용
    - **게이지 충전 속도**: 초당 보유 Corpse의 50% (예: 10개 보유 시 2초에 게이지 100%)
    - **최소 사용량**: 게이지가 1 Corpse 이상 차야 발동 가능
    - **게이지 취소**: Explode 게이지 충전 중 다시 E 키를 누르면 취소 (Corpse 소비 없음)
    - **게이지 중 이동**: 가능 (이동속도 -30% 패널티)
...
| ----------- | ----------- | --------------------------- |
| 0~1 유닛    | ×1.0 (100%) | —                           |
| 1~2 유닛    | ×0.7 (70%)  | 구간 내 선형 보간 (1.0→0.7) |
| 2 유닛~최대 | ×0.4 (40%)  | 구간 내 선형 보간 (0.7→0.4) |

> **피해 계산 공식**: `damage = baseDamage × lerp(구간시작배율, 구간종료배율, (거리 - 구간시작) / 구간길이)`
...
- 바닥에 있는 **미수집 Corpse 위에서 폭발**하면, 해당 Corpse도 추가 폭발에 포함 (연쇄 폭발)
    ...
    - 연쇄 폭발 반경: 바닥 Corpse 위치 중심 2 유닛 (거리 감소 없이 균일 피해)
    - 연쇄 폭발은 **1회만 전파** (연쇄→연쇄로 다시 전파되지 않음)
    - 연쇄 폭발된 Corpse는 소멸 (수집 불가)
    - 연쇄 폭발과 본 폭발이 동일 적에게 적중 시 **피해 합산** (별개 히트 판정)
- 보스에게는 Explode 피해가 **70%만 적용** (보스 내성, 연쇄 폭발 포함)

---

## Corpse 반환 — 미니언 해제

미니언을 [해제(Dismiss)](./minion-system.md)하면 소환 비용의 일부를 Corpse로 돌려받는다.

| 구분      | 반환 비율 | 비고                                      |
| --------- | --------- | ----------------------------------------- |
| T1/T2     | 50%       | 소환 비용의 절반 (반올림)                 |
| T3        | 25%       | 합성 총 비용 기준 (높은 투자 → 낮은 반환) |
| 최소 반환 | 0         | 비용 1 미니언은 반환 없음                 |

> 미니언 해제는 군단 구성 변경의 유연성을 제공하되, 반환율이 낮아 신중한 소환 판단을 유도한다. 상세: [미니언 시스템 → 미니언 해제](./minion-system.md)
...
```
  ...
  스켈레톤 ×10    그래브 나이트 ×2(합성)   250 데미지 광역
  즉시 전력 보충   소수 정예(+15% 보너스)    긴급 탈출
         │              │                       │
    물량 전략       정예 전략                생존 전략
                        │
                   기존 미니언 해제 → 슬롯 확보 + 일부 Corpse 회수
```
