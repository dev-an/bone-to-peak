...
| 입력              | 동작                                       |
| ----------------- | ------------------------------------------ |
| Tab               | 미니언 목록 / 군단 정보 (미니언 해제 가능) |
| ESC               | 일시정지 메뉴                              |
| Tab → 미니언 클릭 | 선택된 미니언 상세 정보 → 해제 버튼        |
...
바이옴 선택 후, 런 진입 직전에 무기 선택 화면이 표시된다.
...
- **사망 분석(Death Recap)** 화면 표시 후 런 결과 화면으로 이동
- 결과 화면에서 메타 강화 상점 또는 재시작 선택

### 사망 분석 (Death Recap)

사망 직후 슬로모션 연출이 끝나면, 결과 화면 전에 사망 원인을 분석하는 화면이 표시된다.
...
| 항목              | 설명                                                 |
| ----------------- | ---------------------------------------------------- |
| 사망 원인         | 마지막 피해를 가한 적 유형과 공격 패턴               |
| 최근 피해 로그    | 사망 전 5초간 받은 피해를 시간순으로 표시            |
| 런 전체 피해 비율 | 카테고리별 누적 피해 비율 (적 유형, 패턴, 환경)      |
| 전술 팁           | 사망 원인에 기반한 대응 조언 (포메이션, 회피 등)     |
| 데이터 기록       | 사망 분석 데이터는 런 결과 + 도감 시스템에 누적 반영 |

### 자동 일시정지

| 조건                   | 동작                               |
| ---------------------- | ---------------------------------- |
| 게임 창 포커스 손실    | 자동 일시정지 (`Time.timeScale=0`) |
| 포커스 복귀            | 일시정지 메뉴 표시 (자동 재개 X)   |
| 레벨업 선택지 표시 중  | 이미 일시정지 상태                 |
| 설정에서 비활성화 가능 | "포커스 손실 시 일시정지" 토글     |
