...
| 고유 효과 1 | 묘지 토양 — Corpse 바닥 유지 시간 +1초 (6초)                    |
| 고유 효과 2 | 안식의 땅 — 미니언 사망 시 20% 확률로 Corpse 1개 드롭 (리치 외) |
...
| 고유 효과 1 | 산화된 무기 — 엘리트 적 스폰 확률 +5% (기본 10% → 15%)           |
| 고유 효과 2 | 전사의 유산 — 적 처치 시 5% 확률로 임시 버프 드롭 (기본 1% → 5%) |
| 색조        | 잿빛 + 짙은 적색 포인트                                          |
...
| 고유 효과 1 | 네크로 공명 — Raise 쿨타임 -0.3초                |
| 고유 효과 2 | 마력 집중 — Refine 합성 소요 시간 -30%           |
| 색조        | 짙은 보라 + 녹색 마법 발광                       |
...
웨이브 진행에 따라 맵에 **동적 변화**가 발생하여 전술 상황이 달라진다.

### 이벤트 목록

| 이벤트        | 발동 시점        | 효과                                                       | 지속 시간 |
| ------------- | ---------------- | ---------------------------------------------------------- | --------- |
| 안개 확산     | Wave 8           | 죽음의 안개가 4 유닛 안쪽으로 확장 → 활동 영역 축소        | 영구      |
| 지형 붕괴     | Wave 12          | 랜덤 대형 장애물 1개 파괴 → 새 경로 개방 + Corpse 3개 드롭 | 영구      |
| 네크로 폭풍   | Wave 15          | 맵 전체에 10초간 Corpse 비 (랜덤 위치에 Corpse 15개 투하)  | 10초      |
| 안개 2차 확산 | Wave 18          | 안개 추가 3 유닛 확장 → 보스전을 위한 전장 긴장감 강화     | 영구      |
| 최후의 무대   | Wave 20 (보스전) | 안개 확산 정지, 맵 중심에 네크로 원천 1개 추가 출현        | 보스전간  |

### 이벤트 연출

| 이벤트      | 시각 연출                                              | 사운드          |
| ----------- | ------------------------------------------------------ | --------------- |
| 안개 확산   | 안개가 2초에 걸쳐 서서히 전진, 경계선 붉은색 경고 표시 | 저주파 윙 소리  |
| 지형 붕괴   | 화면 흔들림 0.3초 + 먼지 파티클                        | 돌 무너지는 SFX |
| 네크로 폭풍 | 하늘에서 녹색 빛줄기가 떨어지며 Corpse 투하            | 바람 + 마법 SFX |

> 동적 맵 이벤트는 런마다 동일하게 발생한다 (랜덤 아님). 플레이어가 이벤트 타이밍을 학습하고 준비할 수 있도록 의도적으로 고정한다.
