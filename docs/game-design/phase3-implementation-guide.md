...
```
Step 1 (독립 구현 가능)
├── 1A. 포메이션 시스템 (Guard 상태, 3가지 포메이션, 역할 기반 배치)
├── 1B. 플레이어 기본 공격 (지팡이/소울 애로우, 무기 선택 UI)
└── 1C. 경험치/레벨업 시스템 (경험치 구슬, 레벨업 UI)

Step 2 (Step 1 완료 후)
├── 2A. Refine 합성 시스템 (Step 1A 포메이션 필요, 합성 보너스 +15%)
├── 2B. Explode 시체 폭발 (독립적이지만 Corpse 시스템 확장)
└── 2C. 미니언 해제 시스템 (Tab 메뉴, Corpse 반환)

Step 3 (Step 2 완료 후)
├── 3A. 엘리트 적 (기사, 마법사, 암살자) + 미니보스
└── 3B. 보스 시스템 (성기사 → 드래곤 → 대마법사 → 용사)

Step 4 (Step 3 완료 후)
├── 4A. UI/HUD 통합 (모든 시스템의 이벤트를 UI에 연결)
├── 4B. 사망 분석 (Death Recap) 화면
└── 4C. 튜토리얼 시스템 (첫 런 안내 팝업)

Step 5 (Step 4 완료 후)
├── 5A. 엔드리스 모드 (Wave 20+ 무한 스케일링)
├── 5B. 도감 시스템 (Bestiary)
└── 5C. 동적 맵 이벤트 (안개 확산, 지형 붕괴 등)
```
...
1. `FormationManager` — 포메이션 전환 관리, 슬롯 좌표 계산, **역할 기반 슬롯 배정**
2. `MinionBase` 확장 — Guard 상태 추가, 포메이션 보너스/패널티 적용, **역할 분류 (Front/Mid/Back)**
...
- 합성 완료 → 재료 사라짐 + 결과 미니언 등장 (**합성 보너스 +15% 적용**)
- 합성 중 피격 → 합성 취소 (재료 보존, Corpse 반환)
- T3 레시피 해금: 재료 미니언 모두 동시 보유 이력 확인
- `MinionBase.IsRefined` 플래그: 합성으로 생성된 미니언 구분, 스탯 보너스 적용
...
public static event Action<MinionStatsSO> OnMinionDismissed;        // 미니언 해제
public static event Action<int> OnEndlessWaveStarted;               // 엔드리스 웨이브 시작
public static event Action<string> OnBestiaryEntryUnlocked;         // 도감 항목 해금
public static event Action<DamageRecord> OnPlayerDamaged;           // 플레이어 피격 (사망분석용)
public static event Action<int> OnMiniBossSpawned;                  // 미니보스 등장
```
...
> 기획 문서: [minion-system.md](./minion-system.md), [corpse-economy.md](./corpse-economy.md)

### 구현 범위

1. `DismissSystem` — 미니언 해제 로직, Corpse 반환 계산
2. 군단 정보 UI에 해제 버튼 추가
...
Assets/Scripts/
├── Player/
│   └── DismissSystem.cs           ← 신규: Tab 메뉴에서 미니언 해제
├── Minions/
│   └── MinionBase.cs              ← 수정: Dismiss() 메서드 추가
├── UI/
│   └── MinionListUI.cs            ← 신규: Tab 키 군단 정보 + 해제 버튼
```

### 핵심 구현 사항

- Tab 키로 군단 정보 패널 열기 (게임 일시정지 아님)
- 미니언 클릭 → 상세 정보 + "해제" 버튼
- 해제 시 Corpse 반환: T1/T2 = 소환 비용 ×50%, T3 = 합성 총 비용 ×25%
- 해제 애니메이션: 0.3초 청백색 소멸 이펙트 (사망과 구분)
- 합성 중 재료 미니언은 해제 불가
...
| 파일              | 변경 내용                                      |
| ----------------- | ---------------------------------------------- |
| `GameEvents.cs`   | `OnMinionDismissed(MinionStatsSO)` 이벤트 추가 |
| `MinionBase.cs`   | `Dismiss()` 메서드 추가 (슬롯 반환 + 풀 반환)  |
| `PlayerCombat.cs` | Corpse 반환 로직 연동                          |
...
1. `EndlessManager` — Wave 20 이후 스케일링, 미니보스 스폰
2. `EnemySpawner` 확장 — 엔드리스 스폰 로직
...
Assets/Scripts/
├── Core/
│   └── EndlessManager.cs          ← 신규: 엔드리스 웨이브 관리
├── Enemies/
│   └── EnemySpawner.cs            ← 수정: 엔드리스 스폰 로직 추가
```

### 핵심 구현 사항

- Wave 20 클리어 시 선택 UI: "엔드리스 도전" / "결과 화면으로"
- 엔드리스 스케일링: `baseMultiplier × 1.15^(wave - 20)`
- 5웨이브마다 미니보스 (기존 보스 2~3종 동시, 스케일링 적용)
- Corpse 수용량 5웨이브마다 +2 (무한)
- 레벨 상한 해제 (레벨 20+ 가능)
...
> 기획 문서: [progression-system.md](./progression-system.md)
...
Assets/Scripts/
├── Core/
│   └── BestiaryManager.cs         ← 신규: 도감 관리, PlayerPrefs 저장
├── UI/
│   └── BestiaryUI.cs              ← 신규: 메인 메뉴 도감 화면
```
...
> 기획 문서: [level-map-design.md](./level-map-design.md)
...
Assets/Scripts/
├── Core/
│   └── MapEventManager.cs         ← 신규: 웨이브 기반 맵 이벤트 트리거
```

### 핵심 구현 사항

- 웨이브 번호 기반 이벤트 발동 (고정 타이밍)
- 안개 확산: 경계 Collider 크기 조절
- 지형 붕괴: 장애물 파괴 + Corpse 스폰
- 네크로 폭풍: 랜덤 위치에 Corpse 오브젝트 스폰
