...
| 자동 소환      | 가장 최근 미니언을 쿨타임마다 자동 소환 (Corpse 소비) | 2 (1회 한정)     |
...
| 미니언 기본 공격력 | 5    | 25 / 50 / 100 / 200 / 400  | 전체 미니언 공격력 +3%     |
| Corpse 수용량 증가 | 3    | 20 / 40 / 80               | 시작 Corpse 수용량 +2      |
| 소환 쿨타임 감소   | 3    | 30 / 60 / 120              | 소환 쿨타임 -0.1초         |
| 네크로 코인 보너스 | 5    | 50 / 100 / 200 / 400 / 800 | 런 종료 시 네크로 코인 +5% |
...
최종 보스(웨이브 20)를 클리어하면 **Ascension 레벨**이 해금된다. 총 10단계로 구성된다.
...
| 6         | 적 HP +200%, Corpse 바닥 유지 시간 3초로 단축 (기본 5초), 미니보스 매 웨이브 |
| 7         | 적 HP +260%, 적 공격력 +120%, 엘리트 스폰 확률 35%                           |
| 8         | 적 HP +330%, 보스 면역 강화 (Explode 50%), 레벨업 선택지 2개로 감소          |
| 9         | 적 HP +400%, 적 공격력 +180%, 네크로 원천 효과 비활성화                      |
| 10        | 적 HP +500%, 모든 스탯 +250%, "정점의 시련" — 사망 시 기록 특별 표시         |
...
| 5         | 네크로맨서 스킨: 리치 킹 + 칭호: "불멸자" | 전용 이펙트 포함                 |
| 6         | 미니언 외형: 심연의 군단                  | 전체 미니언 보라색 이펙트        |
| 7         | 칭호: "뼈의 지배자" + 네크로 코인 ×1000   | 대량 코인                        |
| 8         | 네크로맨서 스킨: 심연의 지배자            | 검은 오라 + 붉은 눈 이펙트       |
| 9         | 칭호: "망자의 황제" + 네크로 코인 ×2000   |                                  |
| 10        | 네크로맨서 스킨: 정점의 신 + 칭호: "신"   | 최종 보상, 모든 이펙트 금색 변환 |
...
> **네크로 코인 최종 계산**: `기본 공식 × 등급 배율 × Ascension 보너스(Ascension 레벨 × 0.2 + 1.0) × 메타 코인 보너스`

---

## 도감 시스템 (Bestiary)

메인 메뉴에서 접근 가능한 도감. 게임 플레이 중 발견한 적, 미니언, 보스를 기록한다.

### 도감 카테고리

| 카테고리  | 항목                                 | 해금 조건             |
| --------- | ------------------------------------ | --------------------- |
| 일반 적   | 스웜, 할버디어, 궁수, 방패병         | 해당 적과 첫 조우     |
| 엘리트 적 | 기사, 마법사, 암살자                 | 해당 엘리트와 첫 조우 |
| 보스      | 성기사, 드래곤, 대마법사, 용사       | 해당 보스와 첫 조우   |
| T1 미니언 | 스켈레톤 워리어, 좀비, 스켈레톤 아처 | 해당 미니언 첫 소환   |
| T2 미니언 | 그래브 나이트, 레이스, 본 골렘       | 해당 미니언 첫 소환   |
| T3 미니언 | 데스 나이트, 본 드래곤, 리치         | 해당 미니언 첫 합성   |
...
│  총 처치 수: 47                      │
│  총 받은 피해: 1,230                 │
│  첫 조우: Wave 5 (2026-02-28)        │
│                                      │
│  💡 대응 팁: Phalanx로 돌진을 차단,  │
│    넉백으로 거리 유지                 │
└──────────────────────────────────────┘
```
...
- 전체 도감 완성률이 메인 메뉴에 표시 (예: "도감 14/17")
- 미발견 항목은 실루엣 + "???" 표시
- 도감 100% 완성 시 특별 칭호: "기록자" 해금

### 데이터 저장

| 항목         | 저장 방식                                |
| ------------ | ---------------------------------------- |
| 해금 상태    | `PlayerPrefs` `Bestiary_{ID}_Unlocked`   |
| 누적 통계    | `PlayerPrefs` `Bestiary_{ID}_{StatType}` |
| 첫 조우 날짜 | `PlayerPrefs` `Bestiary_{ID}_FirstMet`   |

---

## 런 통계 추적

매 런마다 상세 통계를 기록하며, 누적 데이터는 도감과 메타 시스템에 반영된다.

### 런별 추적 항목

| 카테고리 | 항목                                                             |
| -------- | ---------------------------------------------------------------- |
| 전투     | 적 처치 수 (유형별), 보스 처치 수, 받은 총 피해, 최대 연속 킬    |
| 자원     | Corpse 수집량, Raise 횟수, Refine 횟수, Explode 횟수/총 피해     |
| 군단     | 최대 동시 미니언, 미니언 사망 수, T3 합성 횟수, 미니언 해제 횟수 |
| 성장     | 최종 레벨, 선택한 스킬 목록, 포메이션 전환 횟수                  |
| 생존     | 생존 시간, 사망 원인, 최종 웨이브, 무피격 최장 시간              |

### 역대 기록 (All-Time Records)

메인 메뉴에서 확인 가능한 누적 기록.

| 기록                | 설명                               |
| ------------------- | ---------------------------------- |
| 최고 웨이브         | 도달한 최고 웨이브 (엔드리스 포함) |
| 최다 처치           | 한 런에서의 최다 적 처치           |
| 최장 생존           | 한 런 최장 생존 시간               |
| 최대 미니언         | 동시 소환 최대 미니언 수           |
| 최대 Explode 피해   | 단일 Explode 최대 피해             |
| 총 처치 수          | 전체 런 누적 처치                  |
| 총 플레이 시간      | 전체 런 누적 시간                  |
| Ascension 최고 레벨 | 클리어한 최고 Ascension            |
