...
T2 미니언은 T1보다 스탯이 높을 뿐 아니라, 각각 고유한 패시브 능력을 보유한다.

| 미니언        | 패시브 이름 | 효과                                                               |
| ------------- | ----------- | ------------------------------------------------------------------ |
| 그래브 나이트 | 사기 진작   | 주변 2 유닛 내 T1 미니언 공격력 +10% (중첩 불가)                   |
| 레이스        | 영혼 포식   | 적 처치 시 30% 확률로 자신의 HP 10 회복                            |
| 본 골렘       | 뼈의 방벽   | 자신이 공격받을 때 주변 1.5 유닛 내 미니언이 받는 피해 -3 (고정값) |
...
[Refine(합성)](./corpse-economy.md#refine--합성)으로 생성된 미니언은 직접 Raise로 소환한 미니언보다 강하다. 합성이라는 추가 노력(시간, 위험, 재료 미니언 손실)에 대한 보상이다.

### 합성 보너스 규칙

| 항목               | 값                                                     |
| ------------------ | ------------------------------------------------------ |
| 합성 보너스 배율   | 전체 스탯 +15% (HP, 공격력, 이동속도)                  |
| 적용 대상          | Refine으로 생성된 T2 미니언                            |
| T3 (Peak-Guardian) | 합성 전용이므로 기본 스탯이 이미 합성 보너스를 포함    |
| 시각 표현          | 합성 미니언 이름 옆에 ✦ 표시, 약간 밝은 외곽선         |
| 스탯 표시          | Tab 키 군단 정보에서 "(합성)" 태그 및 보너스 수치 표시 |

### 합성 보너스 적용 예시

| 미니언                 | 기본 HP | 합성 HP | 기본 공격력 | 합성 공격력 |
| ---------------------- | ------- | ------- | ----------- | ----------- |
| 그래브 나이트 (Raise)  | 100     | —       | 18          | —           |
| 그래브 나이트 (Refine) | —       | 115     | —           | 20.7 (→21)  |
| 레이스 (Raise)         | 40      | —       | 25          | —           |
| 레이스 (Refine)        | —       | 46      | —           | 28.75 (→29) |
| 본 골렘 (Raise)        | 180     | —       | 10          | —           |
| 본 골렘 (Refine)       | —       | 207     | —           | 11.5 (→12)  |

> **설계 의도**: Refine은 직접 Raise보다 Corpse 효율이 약간 낮지만, 합성 보너스(+15%)와 슬롯 절약으로 보상된다. 슬롯이 부족한 중·후반에 합성의 가치가 극대화된다.

---

## 미니언 해제 (Dismiss)

소환된 미니언을 자발적으로 해제하여 **소환 슬롯을 회수**할 수 있다. 군단 구성을 변경하거나 합성 재료를 확보할 때 사용한다.

### 해제 규칙

| 항목            | 값                                               |
| --------------- | ------------------------------------------------ |
| 조작            | Tab 키 (군단 정보) → 미니언 클릭 → 해제 버튼     |
| 슬롯 반환       | 즉시 (해제 애니메이션 중에도 슬롯 사용 가능)     |
| Corpse 반환     | 소환 비용의 50% (반올림, 최소 0)                 |
| 해제 애니메이션 | 0.3초 (소멸 이펙트, 사망과 다른 청백색 이펙트)   |
| 쿨타임          | 없음 (연속 해제 가능)                            |
| 합성 중 해제    | 합성 재료 미니언은 해제 불가 (합성 취소 후 가능) |
| 전투 중 해제    | 가능 (Tab 메뉴 열림 = 게임 일시정지 아님)        |

### 해제 Corpse 반환 테이블

| 미니언          | 소환 비용 | 해제 시 반환 | 비고                              |
| --------------- | --------- | ------------ | --------------------------------- |
| 스켈레톤 워리어 | 1         | 0            | 최저 비용이므로 반환 없음         |
| 좀비            | 1         | 0            |                                   |
| 스켈레톤 아처   | 2         | 1            |                                   |
| 그래브 나이트   | 3         | 2            |                                   |
| 레이스          | 4         | 2            |                                   |
| 본 골렘         | 5         | 3            |                                   |
| 데스 나이트     | (합성)    | 3            | 합성 총 비용(12)의 25%            |
| 본 드래곤       | (합성)    | 5            | 합성 총 비용(19)의 25%            |
| 리치            | (합성)    | 4            | 합성 총 비용(18)의 25%, 드롭 별도 |

> **설계 의도**: 해제는 군단 구성 변경의 유연성을 제공하되, Corpse 반환율을 50%(T1/T2) 또는 25%(T3)로 제한하여 신중한 소환 판단을 유도한다. 해제가 "무료"면 소환의 무게감이 사라진다.

---

## 미니언 포메이션 배치 우선순위

[포메이션](./formation-system.md)에서 미니언의 슬롯 배정은 **역할 기반 우선순위**를 따른다. 탱커는 전방, 원거리 딜러는 후방에 자동 배치된다.

### 배치 역할 분류

| 역할         | 해당 미니언                               | 배치 우선순위    |
| ------------ | ----------------------------------------- | ---------------- |
| 전위 (Front) | 좀비, 그래브 나이트, 본 골렘, 데스 나이트 | 슬롯 0~N (전방)  |
| 중위 (Mid)   | 스켈레톤 워리어                           | 슬롯 N~M (중간)  |
| 후위 (Back)  | 스켈레톤 아처, 레이스, 본 드래곤, 리치    | 슬롯 M~끝 (후방) |

### 배치 공식

```
포메이션 슬롯 배정:
1. 현재 소환된 미니언을 역할별로 분류
2. 전위 → 중위 → 후위 순서로 슬롯 번호 배정
3. 같은 역할 내에서는 티어가 높은 미니언이 앞 슬롯

예시 (Defensive Phalanx, 미니언 8마리):
  슬롯 0~1: 좀비 ×2 (전위)
  슬롯 2~4: 스켈레톤 워리어 ×3 (중위)
  슬롯 5~7: 스켈레톤 아처 ×3 (후위)

Phalanx: 전위가 가장 앞 행
Charge: 전위가 쐐기 선두
Circular: 전위가 적 접근 방향, 후위가 반대편
```

> 자세한 슬롯 좌표 계산은 [포메이션 시스템 → 슬롯 좌표 계산](./formation-system.md)을 참고한다.
